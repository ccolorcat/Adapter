...
/**
 * 原作者地址：https://github.com/chenpengfei88/StickyItemDecoration
 */
...
//        mStickyView = new ExampleStickyView();
    ...
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        for (int m = 0, size = parent.getChildCount(); m < size; m++) {
             ...
             * 如果是吸附的view
             */
            RecyclerView.ViewHolder holder = findViewHolder(parent, view);
            if (mStickyView.isStickyView(holder)) {
                mCurrentUIFindStickView = true;
                getStickyViewHolder(parent, holder);
    ...
    private View getNextStickyView(RecyclerView parent) {
        ...
        for (int m = 0, size = parent.getChildCount(); m < size; m++) {
            View view = parent.getChildAt(m);
            if (mStickyView.isStickyView(findViewHolder(parent, view))) {
                nextStickyView = view;
                num++;
            }
            if (num == 2) break;
     ...
     * @param recyclerView
     */
    private void getStickyViewHolder(RecyclerView recyclerView, RecyclerView.ViewHolder holder) {
        if (mAdapter != null) return;

        mAdapter = recyclerView.getAdapter();
        mViewHolder = mAdapter.onCreateViewHolder(recyclerView, holder.getItemViewType());
    ...
    private static RecyclerView.ViewHolder findViewHolder(RecyclerView recyclerView, View itemView) {
        return recyclerView.getChildViewHolder(itemView);
    }
}
